...
void display(char *fname)
{
	...
	{ // display file contents to standard output
		...
		int inode_to_display = e_inode; // aa inode no XX,YY,ZZ no data display karvano che
		int fileblocks[3];
		fileblocks[0] = stoi(_inode_table[e_inode].XX, 2);
		fileblocks[1] = stoi(_inode_table[e_inode].YY, 2);
		fileblocks[2] = stoi(_inode_table[e_inode].ZZ, 2);

		for (i = 0; i < 3; i++)
		{
			if (fileblocks[i] == 0)
				continue; // 0 means pointing at nothing
			readSFS(fileblocks[i], (char *)read_buffer);
...
void create(char *fname)
{
	...
	for (i = 0; i < 3; i++)
	{
		...
		readSFS(blocks[i], (char *)_directory_entries); // read a directory entry; notice the cast
		...
		for (j = 0; j < 4; j++)
		{
			...
			{ // means unused entry
				if (empty_dentry == -1)
				{
					empty_dentry = j;
					empty_dblock = i;
				} // lets keep a note of it, just in case we have to create the new file
				continue;
			}
	...
	// if we did not find an empty file entry and all three blocks are in use; then no new file can be made
	if (empty_dentry == -1 && empty_dblock == -1)
	{
		printf("Error: Maximum directory entries reached.\n");
		return;
	...
	{ // otherwise
		...
		for (int i = 0; i < 3; ++i)
		{
			...
			printf("Blocks required now : %c\n", i + 1);
		}

		if (blocks_needed > free_disk_blocks)
		{
			printf("Disk is full!\n");
			return;
		};
		// write content in the block 	1. assign block 	2. write data on the block
		int file_blocks[3];

		file_blocks[0] = getBlock();
		if (blocks_needed > 1)
			file_blocks[1] = getBlock();
		if (blocks_needed > 2)
			file_blocks[2] = getBlock();

		// printing data which is to be written
		...
		// got the blocks, now write in them

		writeSFS(file_blocks[0], (char *)writ_cont[0]);
		if (blocks_needed > 1)
			writeSFS(file_blocks[1], (char *)writ_cont[1]);
		if (blocks_needed > 2)
			writeSFS(file_blocks[2], (char *)writ_cont[2]);

		file_ientry = getInode();
		// empty_ientry is the inode where information of our file will be stored
		//  get an empty place in the inode table which will store info about blocks for this new file.

		readSFS(blocks[empty_dblock], (char *)_directory_entries);	 // read block of current directory where info on this new file will be written
		_directory_entries[empty_dentry].F = '1';					 // remember we found which directory entry is unused well, set it to used now
		strncpy(_directory_entries[empty_dentry].fname, fname, 252); // put the name in there
		itos(_directory_entries[empty_dentry].MMM, file_ientry, 3);	 // and the index of the inode that will hold info inside this directory
		writeSFS(blocks[empty_dblock], (char *)_directory_entries);	 // now write this block back to the disk

		strncpy(_inode_table[file_ientry].TT, "FI", 2); // create the inode entry ,its a file, so FI
		// need to give the inode table, address of the blocks

		itos(_inode_table[file_ientry].XX, file_blocks[0], 2);
		if (blocks_needed > 1)
			itos(_inode_table[file_ientry].YY, file_blocks[1], 2);
		else
			strncpy(_inode_table[file_ientry].YY, "00", 2);
		if (blocks_needed > 2)
			itos(_inode_table[file_ientry].ZZ, file_blocks[2], 2);
		else
			strncpy(_inode_table[file_ientry].ZZ, "00", 2);
...
/*
/****************************************************************************/
void remove_recursive(int current_inode)
{
	char itype;
	int blocks[3];
	_directory_entry _directory_entries[4];
	...
	// traverse the directory entries in all three blocks
	for (i = 0; i < 3; i++)
	{
		if (blocks[i] == 0)
			continue; // 0 means pointing at nothing
